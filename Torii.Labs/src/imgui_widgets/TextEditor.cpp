...
{
	if (aGlyph.mHasCustomColor)
		return aGlyph.mCustomColor;
	if (!mColorizerEnabled)
...
void TextEditor::Render()
{
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			for (const auto& highlight : mLineHighlights)
			{
				if (lineNo < highlight.mStartLine || lineNo > highlight.mEndLine)
					continue;

				auto end = ImVec2(lineStartScreenPos.x + contentSize.x + 2.0f * scrollX, lineStartScreenPos.y + mCharAdvance.y);
				drawList->AddRectFilled(start, end, highlight.mColor);
			}
...
void TextEditor::SetAnsiText(const std::string& aText, bool aScrollToTop)
{
	...
	const auto defaultColor = mPalette[(int)PaletteIndex::Default];
	...
	auto ansi16Color = [&](int code) -> ImU32 {
		switch (code)
		{
		case 30: return rgb(0, 0, 0);
		case 31: return rgb(205, 49, 49);
		case 32: return rgb(13, 188, 121);
		case 33: return rgb(229, 229, 16);
		case 34: return rgb(36, 114, 200);
		case 35: return rgb(188, 63, 188);
		case 36: return rgb(17, 168, 205);
		case 37: return rgb(229, 229, 229);
		case 90: return rgb(102, 102, 102);
		case 91: return rgb(241, 76, 76);
		case 92: return rgb(35, 209, 139);
		case 93: return rgb(245, 245, 67);
		case 94: return rgb(59, 142, 234);
		case 95: return rgb(214, 112, 214);
		case 96: return rgb(41, 184, 219);
		case 97: return rgb(255, 255, 255);
		default: return defaultColor;
		}
	};

	auto ansi256Color = [&](int index) -> ImU32 {
		if (index < 0)
			return defaultColor;
		if (index < 16)
		{
			static const int lookup[] = {30,31,32,33,34,35,36,37,90,91,92,93,94,95,96,97};
			return ansi16Color(lookup[index]);
		}
		if (index < 232)
		{
			...
			const int b = value % 6;
			const auto channel = [](int c) { return c == 0 ? 0 : 55 + c * 40; };
	...
	for (size_t i = 0; i < aText.size();)
	{
		...
		if (chr == '\x1b' && i + 1 < aText.size() && aText[i + 1] == '[')
		{
			i += 2;
			...
			for (; i < aText.size(); ++i)
			{
				const unsigned char control = static_cast<unsigned char>(aText[i]);
				if (std::isdigit(control))
				{
					value = value * 10 + static_cast<int>(control - '0');
					hasValue = true;
				}
				else if (control == ';')
				{
			...
			if (handled)
			{
				...
				{
					const int code = codes[codeIndex];
					switch (code)
					{
					case 0:
					...
					case 94: case 95: case 96: case 97:
						hasCustomColor = true;
						currentColor = ansi16Color(code);
						break;
					case 38:
						if (codeIndex + 1 < codes.size())
						{
							if (codes[codeIndex + 1] == 5 && codeIndex + 2 < codes.size())
							{
								hasCustomColor = true;
								currentColor = ansi256Color(codes[codeIndex + 2]);
								codeIndex += 2;
							}
							else if (codes[codeIndex + 1] == 2 && codeIndex + 4 < codes.size())
							{
								hasCustomColor = true;
								currentColor = rgb(codes[codeIndex + 2], codes[codeIndex + 3], codes[codeIndex + 4]);
								codeIndex += 4;
							}
						}
						break;
			...
			auto& glyph = mLines.back().emplace_back(Glyph('\x1b', PaletteIndex::Default));
			glyph.mHasCustomColor = hasCustomColor;
			glyph.mCustomColor = currentColor;
			++i;
			continue;
		}

		auto& glyph = mLines.back().emplace_back(Glyph(static_cast<Char>(chr), PaletteIndex::Default));
		glyph.mHasCustomColor = hasCustomColor;
		glyph.mCustomColor = currentColor;
		++i;
	}
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
	...
	if (HasSelection())
	{
		if (aChar == '\t' && mState.mSelectionStart.mLine != mState.mSelectionEnd.mLine)
		{
			...
			for (int i = start.mLine; i <= end.mLine; i++)
			{
				...
				{
					for (int spaceIndex = 0; spaceIndex < mTabSize; ++spaceIndex)
						line.insert(line.begin() + spaceIndex, Glyph(' ', TextEditor::PaletteIndex::Background));
					modified = true;
				}
			}
	...
	else
	{
		if (aChar == '\t')
		{
			const int spaces = mTabSize - (coord.mColumn % mTabSize);
			auto& line = mLines[coord.mLine];
			auto cindex = GetCharacterIndex(coord);
			for (int spaceIndex = 0; spaceIndex < spaces; ++spaceIndex)
				line.insert(line.begin() + cindex + spaceIndex, Glyph(' ', PaletteIndex::Default));
			u.mAdded.assign(static_cast<std::size_t>(spaces), ' ');
			SetCursorPosition(Coordinates(coord.mLine, coord.mColumn + spaces));
		}
		else
		{
		char buf[7];
...
void TextEditor::AddLineHighlight(int aStartLine, int aEndLine, ImU32 aColor)
{
	if (aEndLine < aStartLine)
		std::swap(aStartLine, aEndLine);
	if (aStartLine < 0)
		aStartLine = 0;
	if (aColor == 0)
		return;

	mLineHighlights.push_back(LineHighlight{
...
void TextEditor::Backspace()
{
	...
	else
	{
		...
		{
			auto& line = mLines[mState.mCursorPosition.mLine];
			const bool withinLeadingWhitespace =
				std::all_of(
					line.begin(),
					line.begin() + GetCharacterIndex(pos),
					[](const Glyph& glyph) { return glyph.mChar == ' '; });
			if (withinLeadingWhitespace)
			{
				const int currentColumn = pos.mColumn;
				const int targetColumn = std::max(0, ((currentColumn - 1) / mTabSize) * mTabSize);
				const int removeCount = currentColumn - targetColumn;
				auto cindex = GetCharacterIndex(pos) - removeCount;
				if (removeCount > 0 && cindex >= 0)
				{
					u.mRemovedStart = Coordinates(pos.mLine, targetColumn);
					u.mRemovedEnd = pos;
					u.mRemoved.assign(static_cast<std::size_t>(removeCount), ' ');
					line.erase(
						line.begin() + cindex,
						line.begin() + cindex + removeCount);
					mState.mCursorPosition.mColumn = targetColumn;
					mTextChanged = true;
					EnsureCursorVisible();
